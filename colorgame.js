...
node.setAttribute("style","position:absolute;top:"+(dim.h-_H)/2+";left:"+(dim.w-_W)/2+";width:"+_W+";height:"+_H);
...
var newStar = function(n){
    ...
    st.move = function(){
        if(getDots({x:p.x,y:p.y},{x:st.x,y:st.y}).d < st.r){
            score += st.score;
			if(score>bscore){
			localStorage.setItem("bests",score);
...
p.yy = p.y;
var clicked = false;
document.getElementById("canvas1").onclick=function(){clicked = true;};
